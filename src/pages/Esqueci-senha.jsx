import React, { useEffect, useState } from 'react';
import imagemEsqueciSenha from '../assets/images/imagem-esqueci-senha.svg';

import EtapaEmail from '../components/EsqueciSenha/EtapaEmail';
import EtapaCodigo from '../components/EsqueciSenha/EtapaCodigo';
import EtapaNovaSenha from '../components/EsqueciSenha/EtapaNovaSenha';
import EtapaSucesso from '../components/EsqueciSenha/EtapaSucesso';

import { EmailProvider } from "../components/EsqueciSenha/Context/EsqueciSenhaContext.jsx";

const EsqueciSenha = () => {
    useEffect(() => {
        document.title = "Esqueci Senha | CaringU";
    }, []);

    const [etapa, setEtapa] = useState("novaSenha");
    const [novaSenha, setNovaSenha] = useState('');
    const [confirmarSenha, setConfirmarSenha] = useState('');

    const irPraLogin = () => window.location.href = "/login";

    return (
        <EmailProvider>
            <main className="flex flex-row h-screen w-screen">
                {etapa === "email" && (
                    <EtapaEmail onAvancar={() => setEtapa("codigo")} />
                )}
                {etapa === "codigo" && (
                    <EtapaCodigo onAvancar={() => setEtapa("novaSenha")} />
                )}
                {etapa === "novaSenha" && (
                    <EtapaNovaSenha
                        novaSenha={novaSenha}
                        setNovaSenha={setNovaSenha}
                        confirmarSenha={confirmarSenha}
                        setConfirmarSenha={setConfirmarSenha}
                        onAvancar={() => setEtapa("sucesso")}
                    />
                )}
                {etapa === "sucesso" && (
                    <EtapaSucesso irPraLogin={irPraLogin} />
                )}

                <section className="h-screen w-1/2 max-[1450px]:hidden" >
                    <div className="flex justify-end h-full w-full">
                        <img src={imagemEsqueciSenha} alt="Imagem Esqueci Senha" />
                    </div>
                </section>
            </main>
        </EmailProvider>
    );
};

export default EsqueciSenha;
