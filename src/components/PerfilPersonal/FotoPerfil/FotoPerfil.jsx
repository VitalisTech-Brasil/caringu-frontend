...
export default function FotoPerfil({ urlFoto, nomePersonal, onFotoChange }) {
  const [fileName, setFileName] = useState(urlFoto || "");
  ...
  // Sincroniza preview local com prop urlFoto
  useEffect(() => {
    if (urlFoto && urlFoto !== fileName) {
      setFileName(urlFoto);
      setImgErro(false);
    }
  }, [urlFoto, fileName]);
  ...
  const handleFileChange = (event) => {
    ...
    const tamanhoMaximoBytes = 1 * 1024 * 1024; // 1MB
    if (file.size > tamanhoMaximoBytes) {
      toast.custom((t) => (
        <CustomToast
          t={t}
          type="error"
          message="A imagem excede 1MB. Escolha uma menor."
        />
      ));
      event.target.value = "";
      return;
  ...
  const handleCropConfirm = async () => {
    ...
    try {
      ...
      setFileName(novaUrl);
      onFotoChange?.(novaUrl); // Atualizar o contexto no componente pai
    ...
    <>
      <div className="bg-white border-2 border-[#1D2D441C] rounded-lg p-4 sm:p-6 flex flex-col lg:flex-row items-center sm:justify-between gap-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="text-[24px] font-bold text-gray-800">{nomePersonal}</h3>
          </div>
        </div>
        ...
        <div className="flex flex-col items-start justify-start sm:flex-row gap-4">
          ...
          <div className="flex flex-col items-center justify-between min-h-[64px]">
            ...
            <p className="text-[14px] text-gray-500 mt-1">PNG, JPEG, até 1MB</p>
          </div>
            ...
            <div className="bg-white p-4 rounded-lg w-[90%] max-w-sm sm:max-w-md md:max-w-lg relative max-h-[90vh] overflow-y-auto">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mt-4">
                <div className="flex items-center gap-2">
                  <button onClick={() => setZoom((z) => Math.max(1, z - 0.1))} className="px-3 py-1 bg-gray-300 hover:bg-gray-400 rounded text-lg">-</button>
                  <span className="text-sm w-10 text-center">{zoom.toFixed(1)}x</span>
                  <button onClick={() => setZoom((z) => Math.min(3, z + 0.1))} className="px-3 py-1 bg-gray-300 hover:bg-gray-400 rounded text-lg">+</button>
                </div>
                <div className="flex flex-col sm:flex-row gap-2">
                  <button onClick={() => setShowModal(false)} className="px-4 py-2 cursor-pointer bg-gray-300 hover:bg-gray-400 rounded">Cancelar</button>
                  <button onClick={handleCropConfirm} className="px-4 py-2 cursor-pointer bg-[#E96E35] hover:bg-orange-500 text-white rounded">
                    {loading ? (
                      <div className="flex items-center gap-2">
