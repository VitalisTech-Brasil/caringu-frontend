        ...
        <div className="md:border-2 md:border-[#1D2D441A] md:border-solid py-3 md:py-0 rounded-md w-full flex flex-col md:flex-row">
            ...
            <div className=" border-2 border-[#1D2D441A] border-solid md:border-l-2 md:border-r-0 border-t-0 md:border-b-0 flex flex-col w-full md:w-1/2 h-auto md:h-full xl:w-[55%]">
                ...
                <div className="flex flex-col items-center xl:items-start w-full h-auto mt-2 px-0 md:px-3">
                    ...
                    <div className="flex flex-row w-[95%] md:w-full h-auto items-center gap-2 mt-2 md:mt-1">
                        {selectedDates.length === 0 ? (
                            ...
                            <>
                                ...
                                />
                                <label className="text-sm" htmlFor="">Selecionar Todos</label>
                            </>
                        )}
                    </div>
                    ...
                    <div className="w-full h-auto flex flex-col md:flex-row  items-center mt-4 gap-2 pb-4 xl:ml-6 xl:w-auto">
                        <div className="w-[95%] md:w-auto h-auto flex items-center justify-center">
                            <input
                                type="time"
                                value={horarioInput}
                                onChange={e => setHorarioInput(e.target.value)}
                                className="bg-transparent border-2 border-[#1D2D441A] border-solid p-2 rounded-md w-[10rem] md:w-25 xl:w-30"
                            />
                        </div>
                        <div>
                            <Button
                                texto="Salvar Horário Marcados"
                                cor={horarioInput && checkedDates.length > 0 ? "var(--azul-claro)" : "#15171B87"}
                                corTexto="var(--cor-secundaria)"
                                height="2.75rem"
                                fontWeight="500"
                                ariaLabel="Botão de Salvar Horário"
                                type="button"
                                disabled={!(horarioInput && checkedDates.length > 0)}
                                onClick={handleSalvarHorarios}
                                classNameExtra="w-[15rem] md:w-[13rem] xl:w-[14rem]"
                            />
                        </div>
                    </div>
                </div>
